                                    ...
                                    if statement_data[statement_file_name]['Status'] == STATUS_SUCCESS:
                                        if page_index == 0:
                                            if statement_line.strip().startswith("Account name"):
                                                ...
                                                elif "Jane" in statement_line and not "Gear" in statement_line:
        ...
        try:
            if new_data:
                ...
                if len(statement_df) > 0:
                    ...
                    statement_df = statement_df.interpolate(limit_direction='both', limit_area='inside')
        ...
        try:
            ...
            if len(equity_delta_df):
                ...
                for column in equity_current_df.columns:
                    if column.endswith("Price Close"):
                        tickers.append(column.replace(" Price Close", ""))
                data_df = equity_current_df[[ticker + dimension for ticker in tickers for dimension in [
                    " Price Close",
                    " Currency Rate Base",
                ]]]
                data_df.insert(0, "Date", data_df.index.strftime('%Y-%m-%d'))
                data_df = data_df[data_df['Date'] > '2007-01-01'].sort_index(ascending=False)
                self.sheet_write(data_df, DRIVE_URL, {'index': False, 'sheet': 'History', 'start': 'A1', 'replace': True})
                self.print_log("Data has [{}] columns and [{}] rows pre enrichment"
                               .format(len(equity_current_df.columns), len(equity_current_df)))
                tickers = []
                equity_current_expanded_df = equity_current_df.copy().dropna(axis=1, how='all')
                columns_numeric = []
                for column in equity_current_expanded_df.columns:
                    if "Currency Base" not in column:
                        columns_numeric.append(column)
                equity_current_expanded_df[columns_numeric] = equity_current_expanded_df[columns_numeric].apply(pd.to_numeric)
                for column in equity_current_expanded_df.columns:
                    if column.endswith("Price Close"):
                        tickers.append(column.replace(" Price Close", ""))
                ...
                for ticker in tickers:
                    base_currencies = equity_current_expanded_df[ticker + " Currency Base"]
                    ...
                    for column in [" Price Open", " Price High", " Price Low", " Price Close"]:
                        equity_current_expanded_df[ticker + column + " Base"] = \
                            ...
                            equity_current_expanded_df[ticker + " Currency Rate Base"]
                ...
                self.print_log("Data has [{}] columns and [{}] rows post equity enrichment"
                               .format(len(equity_current_expanded_df.columns), len(equity_current_expanded_df)))
                for index in indexes:
                    equity_current_expanded_df[index + " Currency Base"] = "AUD"
                    equity_current_expanded_df[index + " Paid Dividends"] = 0.0
                    equity_current_expanded_df[index + " Currency Rate Base"] = 1.0
                    equity_current_expanded_df[index + " Currency Rate Spot"] = 1.0
                    equity_current_expanded_df[index + " Stock Splits"] = 0.0
                ...
                self.print_log("Data has [{}] columns and [{}] rows post change enrichment"
                               .format(len(equity_current_expanded_df.columns), len(equity_current_expanded_df)))
                equity_current_expanded_df = equity_current_expanded_df.sort_index(axis=1)
                equity_current_expanded_df.to_csv(os.path.abspath("{}/__Equity_Current_Expanded.csv".format(self.input)))
                self.print_log("Data has [{}] columns and [{}] rows post enrichment"
                               .format(len(equity_current_expanded_df.columns), len(equity_current_expanded_df)))
                equity_current_expanded_df.index = pd.to_datetime(equity_current_expanded_df.index)
                tickers = []
                for column in equity_current_expanded_df.columns:
                    if column.endswith("Price Close"):
                        tickers.append(column.replace(" Price Close", ""))
