...
mosquitto_sub -h ${VERNEMQ_HOST_PROD} -p ${VERNEMQ_PORT} -t haas/entity/# -W 1 -v 2>/dev/null | while IFS= read -r LINE; do
  ...
  echo mosquitto_pub -h ${VERNEMQ_HOST_PROD} -p ${VERNEMQ_PORT} -t ${METADATA_TOPIC} -n -r >/dev/null 2>&1 && echo "Entity Metadata publish script flushing the topic [${METADATA_TOPIC}]"
done
...
for METADATA_FILE in $(find "${ROOT_DIR}/src/build/resources/entity_metadata" -name "*.json" -type f); do
  ...
  echo mosquitto_pub -h ${VERNEMQ_HOST_PROD} -p ${VERNEMQ_PORT} -t ${METADATA_TOPIC} -f ${METADATA_FILE} -r >/dev/null 2>&1 && echo "Entity Metadata publish script published to topic [${METADATA_TOPIC}]"
done
