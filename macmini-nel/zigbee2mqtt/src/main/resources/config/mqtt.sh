...
mosquitto_sub -h ${VERNEMQ_HOST} -p ${VERNEMQ_PORT} -F '%t' -t 'haas/entity/#' -W 1 2> /dev/null | while read METADATA_TOPIC; do
  ([[ $(basename $(dirname $(dirname ${METADATA_TOPIC}))) == 0x* ]] || [[ $(basename $(dirname $(dirname ${METADATA_TOPIC}))) == 122105* ]]) && echo ${METADATA_TOPIC} && mosquitto_pub -h ${VERNEMQ_HOST} -p ${VERNEMQ_PORT} -t "${METADATA_TOPIC}" -n -r
done
