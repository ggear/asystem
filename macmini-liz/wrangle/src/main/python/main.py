...
def main(arguments=[]):
    ...
    runtime_errors = 0
    try:
        with open(library.get_file(".profile") if len(arguments) <= 1 else arguments[1], 'r') as profile_file:
            profile_loaded = library.load_profile(profile_file)
    except Exception as exception:
        runtime_errors += 1
        library.print_log("Wrangle", "Error loading profile", exception)
