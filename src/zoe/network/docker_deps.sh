...
echo "COPY src/main/go /asystem/bin/go"
echo ""
echo "#######################################################################################"
...
echo "    'golang:1.21.6-alpine'" && echo ""
echo "#######################################################################################"
EOF
...
docker run --name "$CONTAINER_NAME" --user root --platform linux/x86_64 --entrypoint sh --mount type=bind,source=/Users/graham/Code/asystem/src/zoe/network/src/main/go,target=/asystem/bin/go,readonly -dt 'golang:1.21.6-alpine'
