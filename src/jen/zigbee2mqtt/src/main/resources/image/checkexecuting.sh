...
if
  /asystem/etc/checkalive.sh "${POSITIONAL_ARGS[@]}" && mosquitto_sub -h "$VERNEMQ_SERVICE" -p "$VERNEMQ_API_PORT" -t 'zigbee/bridge/state' -W 1 2>/dev/null | jq -e '.state == "online"' >/dev/null
then
  set +x
