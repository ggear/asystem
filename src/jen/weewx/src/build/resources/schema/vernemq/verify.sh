...
if [ ! -f "${MODULE_DIR}/.env" ]; then
  echo "Schema script [weewx] could not find env file [${MODULE_DIR}/.env]" >&2
  exit 1
fi
set -a
...
topics "homeassistant/+/weewx/#" >> "${RETAINED_FILE}"
topics "weewx/#" >> "${RETAINED_FILE}"
while IFS= read -r TOPIC; do
  [ -z "${TOPIC}" ] && continue
