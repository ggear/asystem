...
var (
	ethernetPort       = schema.Declare("ethernet/port", "switch port health, one row per monitored port", aggregateCadence)
	ethernetName       = ethernetPort.Subject("port", "switch name and port index of the monitored port")
	ethernetUp         = ethernetPort.Bool("up", "port link is live")
	ethernetSpeed      = ethernetPort.Int("speed_mbps", "Mbps", "negotiated link speed")
	ethernetFullDuplex = ethernetPort.Bool("full_duplex", "port negotiated full duplex")
	ethernetDegraded   = ethernetPort.Bool("degraded", "port below expected speed or in half duplex")
	ethernetErrors     = ethernetPort.Int("errors", "", "link errors reported by the port this interval")
)
	...
	for _, device := range devices {
		...
		for _, port := range device.PortTable {
			...
			readings = append(readings, ethernetReading{
				port: key, up: port.Up, speed: int64(port.Speed), fullDuplex: port.FullDuplex, errors: errors})
		}
	}
...
func diagnoseEthernet(samples []plugin.Sample) plugin.Aggregate {
	...
	result.Points = reportEthernet(readings)
	return result
}

func reportEthernet(readings []ethernetReading) []schema.Point {
	points := make([]schema.Point, 0, len(readings))
	for _, reading := range readings {
		points = append(points, ethernetPort.Point(
			ethernetName.Of(reading.port),
			ethernetUp.Of(reading.up),
			ethernetSpeed.Of(reading.speed),
			ethernetFullDuplex.Of(reading.fullDuplex),
			ethernetDegraded.Of(reading.up && ((reading.speed > 0 && reading.speed < expectedSpeed) || !reading.fullDuplex)),
			ethernetErrors.Of(reading.errors)))
	}
	return points
}
