...
func TestZigbee_Report(t *testing.T) {
	devices := []zigbeeReading{
		{name: "coordinator", isCoordinator: true, available: true},
		{name: "kitchen-sensor", lqi: 120, hasLQI: true, available: true},
		{name: "shed-sensor", lqi: 12, hasLQI: true},
	}
	points := reportZigbee(devices)
	if len(points) != len(devices) {
		t.Fatalf("points: got %d want %d (one per device)", len(points), len(devices))
	}
	for index, expected := range []string{"coordinator", "kitchen-sensor", "shed-sensor"} {
		if name, ok := zigbeeName.Read(points[index]); !ok || name != expected {
			t.Errorf("device[%d]: got %q want %q", index, name, expected)
		}
	}
	if coordinator, _ := zigbeeCoordinator.Read(points[0]); !coordinator {
		t.Errorf("coordinator: got false want true")
	}
	if _, ok := zigbeeLQI.Read(points[0]); ok {
		t.Errorf("lqi without a reported value: got set want unset")
	}
	if weak, _ := zigbeeWeak.Read(points[1]); weak {
		t.Errorf("weak[kitchen-sensor]: got true want false above the threshold")
	}
	if weak, _ := zigbeeWeak.Read(points[2]); !weak {
		t.Errorf("weak[shed-sensor]: got false want true below the threshold")
	}
}
