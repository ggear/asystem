...
COMMAND_TOPICS=("network/command/certificate" "network/command/domain" "network/command/ethernet" "network/command/internet" "network/command/weewx" "network/command/wireless" "network/command/zigbee")
...
topics "homeassistant/+/network/+/config" >> "${RETAINED_FILE}"
topics "network/data/#" >> "${RETAINED_FILE}"
while IFS= read -r TOPIC; do
  [ -z "${TOPIC}" ] && continue
