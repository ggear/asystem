	...
	for _, testCase := range tests {
		...
		t.Run(testCase.name, func(t *testing.T) {
			for _, attemptedFormat := range testCase.formats {
				...
				drawErr := display.Draw()
				if (drawErr != nil) != testCase.expectedError {
					fmt.Print("... failed\n")
					t.Fatalf("Draw Display err = %v, expected error? %t", drawErr, testCase.expectedError)
				}
				updateErrs := make(chan error, 1)
				go func() {
					updateErrs <- display.Loop()
				}()
				_ = display.Close()
				select {
				case updateErr := <-updateErrs:
					if (updateErr != nil) != testCase.expectedError {
						fmt.Print("... failed\n")
						t.Fatalf("Loop Display err = %v, expected error? %t", updateErr, testCase.expectedError)
					}
				case <-time.After(250 * time.Millisecond):
					fmt.Print("... failed\n")
					t.Fatalf("Loop Display did not return after close")
				}
