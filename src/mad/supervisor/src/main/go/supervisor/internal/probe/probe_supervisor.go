...
func (p *supervisorProbe) create(_ string, cache *metric.RecordCache, mask [metric.MetricMax]bool, periods config.Periods) error {
	...
	p.supervisorBool = stats.NewBoolStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.versionString = stats.NewStringStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	...
	p.maxMemoryFloat = stats.NewFloatStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.metricRateFloat = stats.NewFloatStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
...
func (p *supervisorProbe) execute(isPulse bool) error {
...
func (p *supervisorProbe) metricCache() *metric.RecordCache {
	return p.cache
}
