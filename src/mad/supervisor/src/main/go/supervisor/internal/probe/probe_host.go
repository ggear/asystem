...
func (p *hostProbe) create(_ string, cache *metric.RecordCache, mask [metric.MetricMax]bool, periods config.Periods) error {
	...
	p.hostBool = stats.NewBoolStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.usedProcessorInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.usedMemoryInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.allocatedMemoryInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.failedServicesInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.failedSharesInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.failedBackupsInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.warnTemperatureInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.spinFanSpeedInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.lifeUsedDrivesInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.usedSystemSpaceInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.usedShareSpaceInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.usedBackupSpaceInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.usedSwapSpaceInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.usedDiskOpsInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.usedNetworkInt = stats.NewIntStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.runningTimeFloat = stats.NewFloatStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)
	p.temperatureFloat = stats.NewFloatStats(periods.TrendHours, periods.PulseSecs, periods.PollSecs)

	return nil
}

func (p *hostProbe) execute(isPulse bool) error {
	executeMetricCacheTasks(p, isPulse, []cacheMetricTask{
		newCacheMetricTask(
			metric.ValueBool,
			metric.MetricHost,
...
func (p *hostProbe) metricCache() *metric.RecordCache {
	return p.cache
}
