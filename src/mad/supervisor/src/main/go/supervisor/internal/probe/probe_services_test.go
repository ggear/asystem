...
func TestProbeServices_Health(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			serviceMap, err := services.services()
			if err != nil {
...
func TestProbeServices_Execute(t *testing.T) {
	tests := []struct {
		name      string
	...
	}{
		{
			...
			checkFunc: func(t *testing.T, p *servicesProbe, cache *metric.RecordCache) {
				if err := p.execute(true); err != nil {
					t.Fatalf("Got execute error = %v, expected nil", err)
				}
				host := config.LocalHostName()
		...
		},
		{
			...
			checkFunc: func(t *testing.T, p *servicesProbe, cache *metric.RecordCache) {
				host := config.LocalHostName()
				if err := p.execute(true); err != nil {
					t.Fatalf("Got execute error on poll 1 = %v", err)
				}
				...
				if err := p.execute(true); err != nil {
					t.Fatalf("Got execute error on poll 2 = %v", err)
				}
				if _, ok := cache.Load(metric.NewServiceRecordGUID(metric.MetricServiceName, host, "svc-a")); ok {
