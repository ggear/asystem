...
func (p *servicesProbe) create(configPath string, cache *metric.RecordCache, mask [metric.MetricMax]bool, periods config.Periods) error {
	...
	p.mask = mask
	p.periods = periods
	p.servicesBool = stats.NewBoolStats(p.periods.TrendHours, p.periods.PulseSecs, p.periods.PollSecs)
	p.servicesMaxMemoryFloat = stats.NewFloatStats(p.periods.TrendHours, p.periods.PulseSecs, p.periods.PollSecs)
...
func (p *servicesProbe) execute(isPulse bool) error {
	servicesByName, err := p.services()
	if err != nil {
	...
	newBool := func() *stats.BoolStats {
		return stats.NewBoolStats(p.periods.TrendHours, p.periods.PulseSecs, p.periods.PollSecs)
	}
	newString := func() *stats.StringStats {
		return stats.NewStringStats(p.periods.TrendHours, p.periods.PulseSecs, p.periods.PollSecs)
	}
	newInt := func() *stats.IntStats {
		return stats.NewIntStats(p.periods.TrendHours, p.periods.PulseSecs, p.periods.PollSecs)
	}
	newFloat := func() *stats.FloatStats {
		return stats.NewFloatStats(p.periods.TrendHours, p.periods.PulseSecs, p.periods.PollSecs)
	}
...
func (p *servicesProbe) metricCache() *metric.RecordCache {
	return p.cache
}
...
func (p *servicesProbe) services() (map[string]service, error) {
	...
	for _, serviceContainer := range containers {
		...
		if err != nil {
			...
			if runningTimeErr != nil {
				...
				service.runningTimeValue = runningTimeValue.Seconds()
			}
...
func runningTime(containerInfo container.InspectResponse) (time.Duration, error) {
