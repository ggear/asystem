...
for _smb in smb smbd; do ! systemctl list-unit-files ${_smb}.service | grep -q masked && systemctl is-active --quiet ${_smb} && systemctl stop ${_smb}; done
...
for _smb in smb smbd; do systemctl list-unit-files ${_smb}.service | grep -q ${_smb}.service && ! systemctl list-unit-files ${_smb}.service | grep -q masked && ! systemctl is-active --quiet ${_smb} && systemctl start ${_smb}; done
