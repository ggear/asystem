...
refresh_setup "$0" "interest" "release"
...
# Touch data files
touch "${REPO_TEST_DIR}/inflation.xlsx" "${REPO_TEST_DIR}/retail.xlsx"
...
awk -F',' -v end_date="${END_DATE}" 'NR==1 || $1 < end_date' "${REPO_TEST_DIR}/__${REPO_TEST_NAME}_current.csv" | tr -d '\r' >"${REPO_TEST_DIR}/__${REPO_TEST_NAME}_current.csv.tmp"
mv "${REPO_TEST_DIR}/__${REPO_TEST_NAME}_current.csv.tmp" "${REPO_TEST_DIR}/__${REPO_TEST_NAME}_current.csv"
ROWS_DELTA=1

# Write fixture toml
FILES_PROCESSED=$(($(find "${REPO_TEST_DIR}" -maxdepth 1 -name "*.xlsx" | wc -l | tr -d ' ')))
