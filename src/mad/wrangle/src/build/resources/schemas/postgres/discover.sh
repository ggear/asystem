...
while [[ $# -gt 0 ]]; do
  ...
  -h | --help | -*)
    ...
    echo "       postgres discover draft a declaration from what production carries"
    exit 2
    ;;
  *)
    shift
...
printf '\n== %s ==\n' "currency"
query_one "SELECT DISTINCT entity, type, period, unit FROM currency ORDER BY entity, type, period, unit" || exit 1
printf '\n== %s ==\n' "equity"
query_one "SELECT DISTINCT entity, type, period, unit FROM equity ORDER BY entity, type, period, unit" || exit 1
printf '\n== %s ==\n' "interest"
query_one "SELECT DISTINCT entity, type, period, unit FROM interest ORDER BY entity, type, period, unit" || exit 1
