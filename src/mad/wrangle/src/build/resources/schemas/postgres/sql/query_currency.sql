...
SELECT
    ...
    entity,
    max(time)                                    AS newest,
    (SELECT max(time) FROM currency) - max(time) AS behind
FROM currency
GROUP BY type, period, unit, entity
HAVING max(time) < (SELECT max(time) FROM currency)
ORDER BY behind DESC
LIMIT 20;

-- snapshot [1d] in [$], yearly shape across entities
SELECT
    time_bucket('1 month', time) AS bucket,
    entity,
    avg(value)                   AS mean,
...
-- delta [1d] in [%], yearly shape across entities
SELECT
    time_bucket('1 month', time) AS bucket,
    entity,
    avg(value)                   AS mean,
...
-- delta [7d] in [%], yearly shape across entities
SELECT
    time_bucket('1 month', time) AS bucket,
    entity,
    avg(value)                   AS mean,
...
-- delta [30d] in [%], yearly shape across entities
SELECT
    time_bucket('1 month', time) AS bucket,
    entity,
    avg(value)                   AS mean,
...
-- delta [365d] in [%], yearly shape across entities
SELECT
    time_bucket('1 month', time) AS bucket,
    entity,
    avg(value)                   AS mean,
