...
FROM equity
WHERE
    type IN ('market-volume-spot', 'price-close', 'price-close-1d-change-percentage', 'price-close-base', 'price-close-base-1d-change-percentage', 'price-close-spot')
...
FROM equity
WHERE
    type IN ('price-close-30d-change-percentage', 'price-close-base-30d-change-percentage', 'price-close-spot-1d-change-percentage', 'price-close-spot-30d-change-percentage')
    AND time >= CURRENT_DATE - INTERVAL '100 day'
    AND time >= (SELECT max(time) FROM equity) - INTERVAL '1 day'
GROUP BY "Bucket", entity
ORDER BY "Bucket", entity;
...
FROM equity
WHERE
    type IN ('price-close-365d-change-percentage', 'price-close-90d-change-percentage', 'price-close-base-90d-change-percentage', 'price-close-spot-90d-change-percentage')
    AND time >= CURRENT_DATE - INTERVAL '100 day'
    AND time >= (SELECT max(time) FROM equity) - INTERVAL '1 day'
GROUP BY "Bucket", entity
ORDER BY "Bucket", entity;
