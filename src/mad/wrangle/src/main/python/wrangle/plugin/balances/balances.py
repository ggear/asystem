...
class Balances(plugin.Plugin):
    ...
    def _run(self):
        ...
        account_metadata_file = abspath(f"{self.local_cache}/_redbark_accounts.json")
        account_metadata = {}
        if isfile(account_metadata_file):
            with open(account_metadata_file) as account_metadata_fh:
                account_metadata = json.load(account_metadata_fh)
        new_data = False
        balance_files = {}
                ...
                if accounts_result.status != plugin.DownloadStatus.FAILED and balances_result.status != plugin.DownloadStatus.FAILED:
                    try:
                        ...
                        for balance_row in balances_raw_df.rows(named=True):
                            ...
                            account_name = account_metadata.get(account_number, str(account.get("name") or ""))
        ...
        balance_downloaded = sum(1 for s in balance_files.values() if s.status == plugin.DownloadStatus.DOWNLOADED)
        balance_cached = sum(1 for s in balance_files.values() if s.status == plugin.DownloadStatus.CACHED)
        self.print_log(f"Files downloaded [{balance_downloaded}] and cached [{balance_cached}]", started=started_time)
        ...
        if new_data:
            try:
                ...
                if len(balances_current_df) and (len(balances_delta_df) or plugin.config.force_reprocessing):
                    ...
                    self.sheet_upload(balances_current_df, self.remote_repos.sheet_key, workbook_name="Bank", sheet_name="Balances", add_filter=True)
