                ...
                if len(statements_positions) == 0:
                    ...
                    for statement_column in statement_df.columns:
                        if statement_column.startswith("Price_"):
                            columns.append("{} Price Close".format(statement_column.replace("Price_", "", 1)))
                        elif statement_column.startswith("Rate_"):
                            columns.append("{} Currency Rate Base".format(statement_column.replace("Rate_", "", 1)))
                        elif statement_column.startswith("Currency_"):
                            columns.append("{} Currency Base".format(statement_column.replace("Currency_", "", 1)))
                        else:
...
from(bucket: "data_public")
    ...
    |> rename(columns: {{_time: "Date", _value: "Rate"}})
                    ...
                    self.database_download(fx_file, fx_query,
                ...
                for ticker in tickers:
                    ...
                    if base_currency == "AUD":
                        ...
                        equity_database_df = equity_database_df.join(fx_rates[base_currency], on="Date", how="full", coalesce=True) \
                ...
                for index in indexes:
                    for column_price in DIMENSIONS_PRICE:
                        ...
                        equity_database_df = equity_database_df.with_columns(
                            pl.sum_horizontal(columns).alias(column))
                        equity_database_df = equity_database_df.with_columns(
