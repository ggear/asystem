...
'
  ...
  while read -r SCHEMA_NAME; do
    ...
    done < <(printf '%s' "${CATALOGUED}" | jq -r --arg s "${SCHEMA_NAME}"       'select(.schema == $s) | [.table, .columns, .megaBytes, .stamp, .epoch] | @tsv')
    if [ -z "${STATEMENT}" ]; then
      printf 'no rows
...
'
    ...
    while IFS=$'	' read -r TABLE SIZE; do
      ...
      STATEMENT="${STATEMENT} ${OLDEST} AS \"oldest\", ${NEWEST} AS \"newest\""
      STATEMENT="${STATEMENT} FROM \"${SCHEMA_NAME}\".\"${TABLE}\""
    done < <(printf '%s' "${HYPERTABLED}" | jq -r --arg s "${SCHEMA_NAME}"       'select(.schema == $s) | [.table, .megaBytes] | @tsv')
    if [ -n "${STATEMENT}" ]; then
