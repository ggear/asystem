...
RESTORE_MODULE_NAME="letsencrypt"
RESTORE_SOURCE_PATH="${RESTORE_SOURCE_PATH:-${SERVICE_DATA_DIR:-/home/asystem/letsencrypt/latest}}"
...
LETSENCRYPT_RESTORE_PATHS="letsencrypt/accounts:letsencrypt/archive:letsencrypt/live:letsencrypt/renewal"

restore_planned() {
  restore_report "would stop [${RESTORE_MODULE_NAME}], extract [${LETSENCRYPT_RESTORE_PATHS}] into [${RESTORE_SOURCE_PATH}] and start it again"
  restore_report "the certificates are pushed to their consumers by [deploy.sh], run that afterwards"
}

restore_applied() {
  restore_files "${LETSENCRYPT_RESTORE_PATHS}"
}
