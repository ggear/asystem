...
var (
	certificateEndpoint        = schema.Declare("certificate/endpoint", "certificate health across the monitored endpoints", aggregateCadence)
	certificateOK              = certificateEndpoint.Bool("ok", "every endpoint verified and none expiring inside the warning window")
	certificateScore           = certificateEndpoint.Int("score", "", "diagnosis score from 0 to 100")
	certificateMinExpiryDays   = certificateEndpoint.Float("min_expiry_days", "d", "days until the nearest certificate expires")
	certificateEndpointsTotal  = certificateEndpoint.Int("endpoints_total", "", "endpoints monitored")
	certificateEndpointsFailed = certificateEndpoint.Int("endpoints_failed", "", "endpoints unreachable or failing verification")
)
...
func diagnoseCertificate(samples []plugin.Sample) plugin.Aggregate {
	...
	stats.ok = result.OK
	result.Points = reportCertificate(stats)
	return result
}

func reportCertificate(stats certificateStats) []schema.Point {
	return []schema.Point{certificateEndpoint.Point(
		certificateOK.Of(stats.ok),
		certificateScore.Of(int64(stats.score)),
		certificateMinExpiryDays.Of(plugin.Round(stats.minDays, 1)),
		certificateEndpointsTotal.Of(int64(stats.reachable+stats.failed)),
		certificateEndpointsFailed.Of(int64(stats.failed)))}
}
