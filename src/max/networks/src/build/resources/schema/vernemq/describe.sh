...
if [ ! -f "${MODULE_DIR}/.env" ]; then
  echo "Schema script [networks] could not find env file [.env] searching up from [${ROOT_DIR}]" >&2
  exit 1
fi
set -a
...
printf '\n== %s ==\n' "homeassistant/+/networks/+/config"
topics "homeassistant/+/networks/+/config"
printf '\n== %s ==\n' "networks/data/#"
topics "networks/data/#"
