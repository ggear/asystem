...
function echo_package_install_commands {
  ...
  echo "docker run -it --rm --user root --entrypoint bash \\
    ...
    'library/influxdb:2.7.10-alpine'" && echo ""
  echo "#######################################################################################"
}
declare -f echo_package_install_commands | sed '1,2d;$d' | docker run --user root --entrypoint bash -i --rm 'library/influxdb:2.7.10-alpine' -