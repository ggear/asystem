...
echo "    'golift/unifi-poller:2.1.3'" && echo ""
echo "#######################################################################################"
EOF
...
docker run --name "$CONTAINER_NAME" --user root --entrypoint sh --mount type=bind,source=/Users/graham/Code/asystem/src/eva/unifipoller/src/main/resources/image,target=/asystem/etc,readonly -dt 'golift/unifi-poller:2.1.3'
