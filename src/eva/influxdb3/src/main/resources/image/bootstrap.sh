...
[ $(curl -sf "http://${INFLUXDB3_SERVICE}:${INFLUXDB3_PORT}/api/v3/configure/database?format=csv&show_deleted=false" | grep -c host_private) -eq 0 ] && echo "Need to create database from outside of host!"
