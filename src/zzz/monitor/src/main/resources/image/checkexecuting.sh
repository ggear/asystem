...
if
  OUTPUT="$(telegraf --test 2>/dev/null)" && [ "$(grep -c '^> cpu,cpu=cpu-total,' <<<"${OUTPUT}")" -gt 0 ] && [ "$(grep -c '^> mem,host=' <<<"${OUTPUT}")" -gt 0 ] && [ "$(grep -c '^> swap,host=' <<<"${OUTPUT}")" -gt 0 ] && [ "$(grep -c '^> diskio,host=' <<<"${OUTPUT}")" -gt 0 ] && [ "$(grep -c '^> net,host=' <<<"${OUTPUT}")" -gt 0 ] && [ "$(grep -c '^> docker_container_cpu,container_image=' <<<"${OUTPUT}")" -gt 0 ] && [ "$(grep -c '^> docker_container_mem,container_image=' <<<"${OUTPUT}")" -gt 0 ] && [ "$(grep -c '^> docker_container_blkio,container_image=' <<<"${OUTPUT}")" -gt 0 ] && [ "$(grep -c '^> docker_container_net,container_image=' <<<"${OUTPUT}")" -gt 0 ]
then
  set +x
  [ "${HEALTHCHECK_VERBOSE}" == true ] && echo "✅ The service [monitor] is executing :)" >&2
  exit 0
else
  set +x
  [ "${HEALTHCHECK_VERBOSE}" == true ] && echo "❌ The service [monitor] is *NOT* executing :(" >&2
  exit 1
fi
