...
class InternetTest(unittest.TestCase):
    ...
    def _test_analyse(self, index, sheet_guid, files_analysed):
        ...
        self.assertEqual(-4, analyse._analyse(join(dir_test, "10/tmp"), sheet_guid, True))
        self.assertEqual(0, analyse._analyse("/share", sheet_guid, False, False, True))
        self.assertEqual(1, analyse._analyse(join(dir_test, "31"), sheet_guid, True))
        self.assertEqual(1, analyse._analyse(join(dir_test, "10/media/docos/movies/The Bad News Bears (1976)"), sheet_guid, True))
        self.assertEqual(1, analyse._analyse(join(dir_test, "10/media/parents/movies/Kingdom of Heaven (2005)"), sheet_guid, True))
        self.assertEqual(1, analyse._analyse(join(dir_test, "10/media/comedy/movies"), sheet_guid, True))
        self.assertEqual(1, analyse._analyse(join(dir_test, "10/media/comedy"), sheet_guid, True))
        self.assertEqual(files_analysed, analyse._analyse(join(dir_test, "10/media"), sheet_guid, True))
        self.assertEqual(files_analysed, analyse._analyse(join(dir_test, "10"), sheet_guid, True))
        self.assertEqual(1, analyse._analyse(join(dir_test, "33"), sheet_guid, True))
        self.assertEqual(1, analyse._analyse(join(dir_test, "33"), sheet_guid, True))
