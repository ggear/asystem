...
BITRATE_SIZE_UPPER_SCALE = 1.3  # Margin when assessing large size
BITRATE_SIZE_LOWER_SCALE = 0.6  # Margin when assessing small size
        ...
        metadata_scripts_pl = metadata_merged_pl.filter(
            ...
            [
                ...
                pl.concat_str([
                    ...
                    pl.lit("      rm -rf \"${ROOT_DIR_BASE}/._metadata_${ROOT_FILE_STEM}\"*.yaml\n"),
                    pl.lit("      rm -rf \"${ROOT_DIR_BASE}/._defaults_analysed_${ROOT_FILE_STEM}\"*.yaml\n"),
                    pl.lit("      rm -rf \"${ROOT_DIR_BASE}/._\"*\"_${ROOT_FILE_STEM}\" >/dev/null 2>/dev/null\n"),
                    pl.lit("      echo '' && echo -n 'Completed: ' && date && exit 0\n"),
                    pl.lit("    else\n"),
                    pl.lit("      echo '' && echo -n 'Failed (mv): ' && date && exit 1\n"),
                    pl.lit("    fi\n"),
