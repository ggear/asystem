...
if [ -n "${SHARE_DIR_MEDIA}" ]; then
  ...
  if [ -f "${SHARE_DIR}/${SCRIPT_FILE}" ]; then "${SHARE_DIR}/${SCRIPT_FILE}" "--verbose" "${SHARE_DIR}/media"; else "${PYTHON_DIR}/python" "${SCRIPT_PATH}" "${SHARE_DIR}/media" "${MEDIA_GOOGLE_SHEET_GUID}" --verbose; fi
else
