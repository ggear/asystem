...
mosquitto_sub -h ${VERNEMQ_SERVICE} -p ${VERNEMQ_PORT} -F '%t' -t 'homeassistant/#' -W 1 2>/dev/null | while read METADATA_TOPIC; do
  ([[ $(basename $(dirname $(dirname ${METADATA_TOPIC}))) == 0x* ]] || [[ $(basename $(dirname $(dirname ${METADATA_TOPIC}))) == 122* ]]) && printf "${METADATA_TOPIC}\n" && mosquitto_pub -h ${VERNEMQ_SERVICE} -p ${VERNEMQ_PORT} -t "${METADATA_TOPIC}" -n -r
done
...
mosquitto_sub -h ${VERNEMQ_SERVICE} -p ${VERNEMQ_PORT} --remove-retained -F '%t' -t 'zigbee/#' -W 1 2>/dev/null
