...
mosquitto_sub -h $VERNEMQ_IP -p $VERNEMQ_PORT --remove-retained -F '%t' -t 'homeassistant/+/weewx/#' -W 1 2>/dev/null
...
mosquitto_sub -h $VERNEMQ_IP -p $VERNEMQ_PORT --remove-retained -F '%t' -t 'weewx/#' -W 1 2>/dev/null
