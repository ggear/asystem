...
from(bucket: "home_private")
|> range(start: v.timeRangeStart, stop: v.timeRangeStop)
|> filter(fn: (r) => r["entity_id"] == "lights_energy_daily" or r["entity_id"] == "fans_energy_daily" or r["entity_id"] == "all_standby_energy_daily" or r["entity_id"] == "coffee_machine_energy_daily" or r["entity_id"] == "pool_filter_energy_daily" or r["entity_id"] == "water_booster_energy_daily" or r["entity_id"] == "dish_washer_energy_daily" or r["entity_id"] == "deck_freezer_energy_daily" or r["entity_id"] == "towel_rails_energy_daily" or r["entity_id"] == "audio_visual_devices_energy_daily" or r["entity_id"] == "servers_network_energy_daily")
|> filter(fn: (r) => r["_field"] == "value")
|> keep(columns: ["_time", "_value", "friendly_name"])
|> aggregateWindow(every: v.windowPeriod, fn: mean, createEmpty: false)
                  '))
