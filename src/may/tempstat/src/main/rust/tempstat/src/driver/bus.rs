...
impl Bus<SerialUart> {
    pub fn open(path: &str, timeout: Duration) -> Result<Self> {
        ...
        Bus::new(SerialUart::open(path, timeout)?)
    }
}
...
impl<U: Uart> Bus<U> {
    ...
    fn detect(&mut self) -> Result<()> {
        ...
        loop {
            ...
            match self.try_detect() {
                Ok(()) => {
                    info!("ds2480b detected on attempt [{attempt}] of [{DETECT_ATTEMPTS}]");
                    return Ok(());
                }
                Err(err) if attempt >= DETECT_ATTEMPTS => return Err(err),
                Err(err) => {
    ...
    pub fn stop_pulse(&mut self) -> Result<()> {
    ...
    pub fn reset(&mut self) -> Result<Presence> {
        self.stop_pulse()?;
        self.set_mode(Mode::Command)?;
    ...
    pub fn write_byte_power(&mut self, byte: u8) -> Result<()> {
        self.set_mode(Mode::Command)?;
    ...
    pub fn get_connected_roms(&mut self) -> Result<Vec<Rom>> {
        self.search(false)
    }

    pub fn alarm_search(&mut self) -> Result<Vec<Rom>> {
        self.search(true)
    }

    pub fn is_connected(&mut self, rom: &Rom) -> Result<bool> {
        match self.reset()? {
        ...
        let seed = rom.bits();
        let (bits, _) = self.search_pass(false, &seed)?;
        Ok(Rom::from_bits(&bits) == *rom)
    }

    fn search(&mut self, alarm: bool) -> Result<Vec<Rom>> {
        ...
        while let Some(seed) = seeds.pop() {
            ...
            let (bits, discrepancies) = self.search_pass(alarm, &seed)?;
    ...
    fn search_pass(&mut self, alarm: bool, seed: &[bool]) -> Result<([bool; 64], [bool; 64])> {
        self.write_byte(if alarm { ROM_SEARCH_ALARM } else { ROM_SEARCH })?;
        self.set_mode(Mode::Command)?;
...
mod tests {
    ...
    fn written(bus: &Bus<MockUart>) -> &[u8] {
    ...
    fn reset_decodes_presence_variants() {
        for (response, presence) in [
            ...
            let mut b = bus();
            b.uart.queue_read(&[response]);
            assert_eq!(b.reset().unwrap(), presence);
            assert_eq!(written(&b), [0xC5]);
        }
    }
    ...
    fn reset_rejects_bad_chip_id() {
        let mut b = bus();
        b.uart.queue_read(&[0xC1]);
        assert!(matches!(b.reset(), Err(Error::InvalidResponse { .. })));
    }

    #[test]
    fn bit_commands_encode_polarity_and_decode_response() {
        let mut b = bus();
        b.uart.queue_read(&[0x97, 0x94, 0x97, 0x84]);
        assert!(b.read_bit().unwrap());
        assert!(!b.read_bit().unwrap());
        b.write_bit(true).unwrap();
        b.write_bit(false).unwrap();
        assert_eq!(written(&b), [0x95, 0x95, 0x95, 0x85]);
    }

    #[test]
    fn bit_commands_reject_malformed_response() {
        let mut b = bus();
        b.uart.queue_read(&[0x17]);
        assert!(matches!(b.read_bit(), Err(Error::InvalidResponse { .. })));
    }

    #[test]
    fn write_bit_rejects_echo_mismatch() {
        let mut b = bus();
        b.uart.queue_read(&[0x94]);
        assert!(matches!(b.write_bit(true), Err(Error::EchoMismatch)));
    }

    #[test]
    fn write_bytes_verifies_echo() {
        let mut b = bus();
        b.uart.queue_read(&[0xAA, 0x55]);
        b.write_bytes(&[0xAA, 0x55]).unwrap();
        assert_eq!(written(&b), [0xE1, 0xAA, 0x55]);
    }

    #[test]
    fn write_bytes_rejects_echo_mismatch() {
        let mut b = bus();
        b.uart.queue_read(&[0xAB]);
        assert!(matches!(b.write_bytes(&[0xAA]), Err(Error::EchoMismatch)));
    }

    #[test]
    fn write_bytes_escapes_command_mode_byte() {
        let mut b = bus();
        b.uart.queue_read(&[0xE3, 0x01]);
        b.write_bytes(&[0xE3, 0x01]).unwrap();
        assert_eq!(written(&b), [0xE1, 0xE3, 0xE3, 0x01]);
    }

    #[test]
    fn read_bytes_sends_read_slots() {
        let mut b = bus();
        b.uart.queue_read(&[0x01, 0x02, 0x03]);
        assert_eq!(b.read_bytes(3).unwrap(), vec![0x01, 0x02, 0x03]);
        assert_eq!(written(&b), [0xE1, 0xFF, 0xFF, 0xFF]);
    }

    #[test]
    fn mode_switches_only_when_needed() {
        let mut b = bus();
        b.uart.queue_read(&[0x11, 0x22, 0x97]);
        b.write_bytes(&[0x11]).unwrap();
        b.write_bytes(&[0x22]).unwrap();
        assert!(b.read_bit().unwrap());
        assert_eq!(written(&b), [0xE1, 0x11, 0x22, 0xE3, 0x95]);
    }
    ...
    fn write_byte_power_arms_strong_pullup() {
        ...
        b.write_byte_power(0x44).unwrap();
        assert_eq!(written(&b), [0x3F, 0x85, 0x85, 0x95, 0x85, 0x85, 0x85, 0x95, 0x87]);
    }
    ...
    fn write_byte_power_rejects_bad_config_response() {
        ...
        assert!(matches!(b.write_byte_power(0x44), Err(Error::InvalidResponse { .. })));
    }
    ...
    fn write_byte_power_rejects_echo_mismatch_and_stops_pulse() {
        ...
        assert!(matches!(b.write_byte_power(0x44), Err(Error::EchoMismatch)));
        assert_eq!(*written(&b).last().unwrap(), 0xF1);
    }
    ...
    fn stop_pulse_terminates_before_next_operation() {
        ...
        b.uart
            .queue_read(&[0x3E, 0x94, 0x94, 0x97, 0x94, 0x94, 0x94, 0x97, 0x94]);
        b.write_byte_power(0x44).unwrap();
        b.uart.queue_read(&[0xF1, 0xCD]);
        assert_eq!(b.reset().unwrap(), Presence::Present);
    ...
    fn stop_pulse_rejects_malformed_response() {
        ...
        b.uart
            .queue_read(&[0x3E, 0x94, 0x94, 0x97, 0x94, 0x94, 0x94, 0x97, 0x94]);
        b.write_byte_power(0x44).unwrap();
        b.uart.queue_read(&[0x00]);
        assert!(matches!(b.stop_pulse(), Err(Error::InvalidResponse { .. })));
    }
    ...
    fn read_rom_returns_validated_rom() {
        ...
        let mut b = bus();
        b.uart.queue_read(&[0xCD, 0x33]);
        b.uart.queue_read(&rom.0);
        assert_eq!(b.read_rom().unwrap(), rom);
        let mut expected = vec![0xC5, 0xE1, 0x33];
        expected.extend([0xFF; 8]);
        assert_eq!(written(&b), expected);
    }
    ...
    fn read_rom_rejects_bad_crc() {
        ...
        b.uart.queue_read(&[0xCD, 0x33]);
        b.uart.queue_read(&rom.0);
        assert!(matches!(b.read_rom(), Err(Error::Crc)));
    }

    #[test]
    fn read_rom_requires_presence() {
        let mut b = bus();
        b.uart.queue_read(&[0xCF]);
        assert!(matches!(b.read_rom(), Err(Error::NoDevice)));
    }
    ...
    fn match_rom_sends_command_and_rom() {
        ...
        echo.extend(rom.0);
        b.uart.queue_read(&echo);
        b.match_rom(&rom).unwrap();
    ...
    fn skip_rom_sends_command() {
        let mut b = bus();
        b.uart.queue_read(&[0xCD, 0xCC]);
        b.skip_rom().unwrap();
        assert_eq!(written(&b), [0xC5, 0xE1, 0xCC]);
    }
    ...
    fn search_finds_single_device() {
        ...
        b.uart.queue_read(&[0xCD, 0xF0]);
        b.uart.queue_read(&response);
        assert_eq!(b.get_connected_roms().unwrap(), vec![rom]);
    ...
    fn search_branches_on_discrepancies() {
        ...
        let mut b = bus();
        b.uart.queue_read(&[0xCD, 0xF0]);
        b.uart.queue_read(&response_a);
        b.uart.queue_read(&[0xCD, 0xF0]);
        b.uart.queue_read(&response_b);
        assert_eq!(b.get_connected_roms().unwrap(), vec![rom_a, rom_b]);
    ...
    fn search_returns_empty_when_bus_absent() {
        let mut b = bus();
        b.uart.queue_read(&[0xCF]);
        assert_eq!(b.get_connected_roms().unwrap(), vec![]);
        assert_eq!(written(&b), [0xC5]);
    }
    ...
    fn search_errors_when_bus_shorted() {
        let mut b = bus();
        b.uart.queue_read(&[0xCC]);
        assert!(matches!(b.get_connected_roms(), Err(Error::Shorted)));
    }
    ...
    fn search_rejects_bad_crc() {
        ...
        b.uart.queue_read(&[0xCD, 0xF0]);
        b.uart.queue_read(&response);
        assert!(matches!(b.get_connected_roms(), Err(Error::Crc)));
    }
    ...
    fn alarm_search_uses_alarm_command() {
        ...
        let mut b = bus();
        let (response, _) = search_response(&[], &[rom]);
        b.uart.queue_read(&[0xCD, 0xEC]);
        b.uart.queue_read(&response);
        assert_eq!(b.alarm_search().unwrap(), vec![rom]);
        assert_eq!(written(&b)[2], 0xEC);
    }
    ...
    fn alarm_search_without_alarming_devices_is_empty() {
        ...
        b.uart.queue_read(&[0xCD, 0xEC]);
        b.uart.queue_read(&response);
        assert_eq!(b.alarm_search().unwrap(), vec![]);
    }
    ...
    fn is_connected_detects_device() {
        ...
        b.uart.queue_read(&[0xCD, 0xF0]);
        b.uart.queue_read(&response);
        assert!(b.is_connected(&rom).unwrap());
    }
