...
printf "\nEntity Metadata publish script [digitemp] dropping discovery topics on [$VERNEMQ_SERVICE]:\n"
...
mosquitto_sub -h $VERNEMQ_SERVICE -p $VERNEMQ_API_PORT --remove-retained -F '%t' -t "homeassistant/+/digitemp/#" -W 5 2>/dev/null

printf "\nEntity Metadata publish script [digitemp] sleeping before dropping data topics ... " && sleep 2 && printf "done\n\n"

printf "Entity Metadata publish script [digitemp] dropping data topics on [$VERNEMQ_SERVICE]:\n"
mosquitto_sub -h $VERNEMQ_SERVICE -p $VERNEMQ_API_PORT -F '%t' -t "telegraf/+/digitemp/#" -W 5 2>/dev/null | sort -u | \
  while read topic; do
    echo "$topic"
    mosquitto_pub -h $VERNEMQ_SERVICE -p $VERNEMQ_API_PORT -t "$topic" -r -n
  done
mosquitto_sub -h $VERNEMQ_SERVICE -p $VERNEMQ_API_PORT --remove-retained -F '%t' -t "telegraf/+/digitemp/#" -W 5 2>/dev/null

printf "\nEntity Metadata publish script [digitemp] sleeping before publishing discovery topics ... " && sleep 2 && printf "done\n\n"

printf "Entity Metadata publish script [digitemp] publishing discovery topics on [$VERNEMQ_SERVICE]:\n"
