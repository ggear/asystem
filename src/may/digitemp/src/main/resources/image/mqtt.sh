...
printf "\nEntity Metadata publish script [telegraf/+/digitemp/#] sleeping before publishing discovery topics ... " && sleep 2 && printf "done\n\n"

printf "Entity Metadata publish script [digitemp] publishing discovery topics on [$VERNEMQ_SERVICE]:\n"
find "$ROOT_DIR" -path "*/digitemp/*" -name "*.json" -print0 | sort -z | while read -d $'\0' METADATA_FILE; do
