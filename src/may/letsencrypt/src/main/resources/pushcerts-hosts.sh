...
ssh -n -q -o "StrictHostKeyChecking=no" root@macbook-lyn "find /var/lib/asystem/install/appdaemon/latest/config -name certificates.sh -exec {} pull macmini-may macbook-lyn \;"
ssh -n -q -o "StrictHostKeyChecking=no" root@macbook-lyn "find /var/lib/asystem/install/appdaemon/latest/config -name certificates.sh -exec {} push macmini-may macbook-lyn \;"
logger -t pushcerts "Pushed new certificates to [macbook-lyn]"
