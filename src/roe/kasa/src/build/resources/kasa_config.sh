...
if netcat -zw 1 10.0.4.93 9999 2>/dev/null; then
	echo '' && echo 'Processing config for device [kitchen_fridge_plug] at [10.0.4.93] ... '
	kasa --host 10.0.4.93 --type plug alias 'Kitchen Fridge Plug'
	kasa --host 10.0.4.93 --type plug led 'True'
else
	echo '' && echo 'Skipping config for device [kitchen_fridge_plug] at [http://10.0.4.93/?] given it is unresponsive'
fi
echo ''