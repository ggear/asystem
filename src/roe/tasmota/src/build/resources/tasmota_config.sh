...
if netcat -zw 1 10.0.4.109 80 2>/dev/null; then
	...
	decode-config.py -s 10.0.4.109 -i /Users/graham/Code/asystem/src/roe/tasmota/src/build/resources/devices/rack_screen_plug.json
	...
	printf '
'
	printf 'Waiting for device to come up .' && sleep 1 && printf '.' && sleep 1 && printf '.' && while ! netcat -zw 1 10.0.4.109 80 2>/dev/null; do printf '.' && sleep 1; done
	printf ' done
'
else
...
if netcat -zw 1 10.0.4.110 80 2>/dev/null; then
	...
	decode-config.py -s 10.0.4.110 -i /Users/graham/Code/asystem/src/roe/tasmota/src/build/resources/devices/rack_backup_plug.json
	...
	printf '
'
	printf 'Waiting for device to come up .' && sleep 1 && printf '.' && sleep 1 && printf '.' && while ! netcat -zw 1 10.0.4.110 80 2>/dev/null; do printf '.' && sleep 1; done
	printf ' done
'
else
