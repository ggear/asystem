...
mosquitto_sub -h $VERNEMQ_SERVICE -p $VERNEMQ_API_PORT --remove-retained -F '%t' -t "supervisor/${SUPERVISOR_HOST}/data/+/+/+" -W 1 2>/dev/null

printf "\nEntity Metadata publish script [supervisor/${SUPERVISOR_HOST}/data/+/+/+] sleeping before publishing discovery topics ... " && sleep 2 && printf "done\n\n"

printf "Entity Metadata publish script [supervisor] publishing discovery topics on [$VERNEMQ_SERVICE]:\n"
find "$ROOT_DIR" -path "*/supervisor/*" -name "*.json" -print0 | sort -z | while read -d $'\0' METADATA_FILE; do
