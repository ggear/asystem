...
mosquitto_sub -h ${VERNEMQ_HOST} -p ${VERNEMQ_PORT} --remove-retained -F '%t' -t 'telegraf/+/internet/#' -W 1 2>/dev/null
mosquitto_sub -h ${VERNEMQ_HOST} -p ${VERNEMQ_PORT} --remove-retained -F '%t' -t 'homeassistant/entity/sensor/internet/#' -W 1 2>/dev/null
