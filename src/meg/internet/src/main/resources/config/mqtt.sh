...
mosquitto_sub -h $VERNEMQ_SERVICE -p $VERNEMQ_PORT --remove-retained -F '%t' -t 'homeassistant/+/internet/#' -W 1 2>/dev/null
...
mosquitto_sub -h $VERNEMQ_SERVICE -p $VERNEMQ_PORT --remove-retained -F '%t' -t 'telegraf/+/internet/#' -W 1 2>/dev/null
