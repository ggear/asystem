            ...
            <div class="forecast clear">
              ${this.forecast.map(daily => html`
                <div class="day fcasttooltip">
                  ...
                  <br><i class="icon" id="fcast-icon-${daily.dayIndex}" style="background: none, url(${this._hass.hassUrl("/local/custom_ui/bom-weather-card/icons/weather_icons/" + (this.config.static_icons ? "static" : "animated") + "/" + this.weatherIcons[this._hass.states[daily.condition].state] + ".svg").replace("-night", "-day")}) no-repeat; background-size: contain;"></i>
...
// #####
  ...
  slotValue(slot,value){
    ...
    var daytimeHigh = this.config.alt_daytime_high ? html`<li><span class="ha-icon"><ha-icon icon="mdi:thermometer-high"></ha-icon></span><span id="alt-daytime-high">${this._hass.states[this.config.alt_daytime_high].state}</span></li>` : this.config.entity_daytime_high ? html`<li><span class="ha-icon"><ha-icon icon="mdi:thermometer-high"></ha-icon></span><span id="daytime-high-text">${this._hass.states[this.config.entity_daytime_high].state}</span><span> ${this.getUOM('temperature')}</span></li>` : ``;
    var daytimeLow = this.config.entity_daytime_low ? html`<li><span class="ha-icon"><ha-icon icon="mdi:thermometer-low"></ha-icon></span><span id="daytime-low-text">${this._hass.states[this.config.entity_daytime_low].state}</span><span> ${this.getUOM('temperature')}</span></li>` : ``;
    var intensity = this.config.entity_pop_intensity ? html`<span id="intensity-text"> ${this._hass.states[this.config.entity_pop_intensity].state.toLocaleString()}</span><span class="unit"> ${this.getUOM('precipitation')}</span>` : ``;
    var pop = this.config.alt_pop ? html`<li><span class="ha-icon"><ha-icon icon="mdi:weather-rainy"></ha-icon></span><span id="alt-pop">${this._hass.states[this.config.alt_pop].state}</span></li>` : this.config.entity_pop ? html`<li><span class="ha-icon"><ha-icon icon="mdi:weather-rainy"></ha-icon></span><span id="pop-text">${this._hass.states[this.config.entity_pop].state}</span><span class="unit"> %</span><span>${intensity}</span></li>` : ``;
    ...
    var wind = this.config.entity_wind_bearing && this.config.entity_wind_speed && this.config.entity_wind_gust ? html`<li><span class="ha-icon"><ha-icon icon="mdi:weather-windy"></ha-icon></span><span>${windBearing}</span><span id="wind-speed-text"> ${this.current.windSpeed}</span>-<span id="wind-gust-text-kt">${this.current.windGust}</span><span class="unit"> ${this.getUOM('length')}/h</span></li>` : ``;
    ...
    var uv_summary = this.config.entity_uv_alert_summary ? html`<li><span class="ha-icon"><ha-icon icon="mdi:weather-sunny"></ha-icon></span><span id="daytime-uv-text">${this._hass.states[this.config.entity_uv_alert_summary].state}</span></li>` : ``;
    var fire_summary = this.config.entity_fire_danger_summary ? html`<li><span class="ha-icon"><ha-icon icon="mdi:fire"></ha-icon></span><span id="daytime-firedanger-text">${this._hass.states[this.config.entity_fire_danger_summary].state}</span></li>` : ``;
...
// #####
  ...
  get localeText() {
    switch (this.config.locale) {
      ...
      default :
        return {
          feelsLike: "Observed on our roof",
...
// #####
  ...
  get current() {
    ...
    var windGust = this.config.entity_wind_gust ? this._hass.states[this.config.entity_wind_gust].state : 0;
    var windSpeedKt = this.config.entity_wind_speed_kt ? Math.round(this._hass.states[this.config.entity_wind_speed_kt].state) : 0;
    var windGustKt = this.config.entity_wind_gust_kt ? this._hass.states[this.config.entity_wind_gust_kt].state : 0;
    var apparent = (this.config.entity_apparent_temp && !this.config.show_decimals) ? this._hass.states[this.config.entity_apparent_temp].state : (this.config.entity_apparent_temp && this.config.show_decimals) ? this._hass.states[this.config.entity_apparent_temp].state.toLocaleString() : 0;
...
// Current Conditions
      ...
      root.getElementById("icon-bigger").style.backgroundImage = `none, url(${this._hass.hassUrl("/local/custom_ui/bom-weather-card/icons/weather_icons/" + (this.config.static_icons ? "static" : "animated") + "/" + this.weatherIcons[this.current.conditions] + ".svg")})`;
...
// Forecast blocks
      this.forecast.forEach((daily) => {
        ...
        root.getElementById("fcast-icon-" + daily.dayIndex).style.backgroundImage = `none, url(${this._hass.hassUrl("/local/custom_ui/bom-weather-card/icons/weather_icons/" + (this.config.static_icons ? "static" : "animated") + "/" + this.weatherIcons[this._hass.states[daily.condition].state] + ".svg").replace("-night", "-day")})`;
...
// Optional Entities
      ...
      if (this.config.entity_pop_intensity && !this.config.entity_pop_intensity_rate) try { root.getElementById("intensity-text").textContent = ` - ${this._hass.states[this.config.entity_pop_intensity].state.toLocaleString()}` } catch(e) {}
      if (this.config.entity_pop_intensity_rate && !this.config.entity_pop_intensity) try { root.getElementById("intensity-text").textContent = ` - ${this._hass.states[this.config.entity_pop_intensity_rate].state.toLocaleString()}` } catch(e) {}
      ...
      if (this.config.entity_daytime_high && !this.config.alt_daytime_high) try { root.getElementById("daytime-high-text").textContent = `${this._hass.states[this.config.entity_daytime_high].state}` } catch(e) {}
      if (this.config.entity_daytime_low) try { root.getElementById("daytime-low-text").textContent = `${this._hass.states[this.config.entity_daytime_low].state}` } catch(e) {}
