...
-- binary_sensor__battery_charging [Home Assistant binary_sensor__battery_charging] every <on-change>, bucketed [1 day] across the newest two buckets
-- part 1 of 1:
SELECT
...
WHERE
    ...
    AND time >= (SELECT max(time) FROM binary_sensor__battery_charging) - INTERVAL '1 day'
