...
SELECT
    ...
    round(avg(auto_update), 1)       AS "Auto Update",
    round(avg(display_precision), 1) AS "Display Precision",
    round(avg(in_progress), 1)       AS "In Progress",
    round(avg(installed_version), 1) AS "Installed Version",
    round(avg(latest_version), 1)    AS "Latest Version",
    round(avg(update_percentage), 1) AS "Update Percentage",
