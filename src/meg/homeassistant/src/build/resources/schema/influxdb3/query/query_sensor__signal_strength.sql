...
-- sensor__signal_strength [Home Assistant sensor__signal_strength] every <on-change>, bucketed [1 day] across the newest two buckets
-- part 1 of 1:
SELECT
...
WHERE
    ...
    AND time >= (SELECT max(time) FROM sensor__signal_strength) - INTERVAL '1 day'
