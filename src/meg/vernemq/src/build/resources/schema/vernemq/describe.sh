...
while [[ $# -gt 0 ]]; do
  ...
  -h | --help | -*)
    ...
    echo "       vernemq describe print what the instance actually holds"
    exit 2
    ;;
  *)
    shift
...
if [ ! -f "${MODULE_DIR}/.env" ]; then
  echo "Schema script [vernemq] could not find env file [.env] searching up from [${ROOT_DIR}]" >&2
  exit 1
fi
set -a
...
BROKER_SERVICE="${BROKER_SERVICE:-${VERNEMQ_BROKER_SERVICE:-${VERNEMQ_SERVICE_PROD:-}}}"
BROKER_PORT="${BROKER_PORT:-${VERNEMQ_BROKER_PORT:-${VERNEMQ_API_PORT:-}}}"

for VARIABLE in BROKER_SERVICE BROKER_PORT; do
  if [ -z "${!VARIABLE}" ]; then
    echo "Schema script [vernemq] could not resolve [${VARIABLE}] from it or any fallback, declare it in the module env files" >&2
    exit 1
  fi
done
