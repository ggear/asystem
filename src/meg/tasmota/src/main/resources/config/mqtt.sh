...
mosquitto_sub -h $VERNEMQ_SERVICE -p $VERNEMQ_PORT --remove-retained -F '%t' -t 'homeassistant/+/tasmota/#' -W 1 2>/dev/null
...
mosquitto_sub -h $VERNEMQ_SERVICE -p $VERNEMQ_PORT --remove-retained -F '%t' -t 'tasmota/#' -W 1 2>/dev/null
