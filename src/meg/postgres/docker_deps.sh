...
echo "    'postgres:14.13-bullseye'" && echo ""
echo "#######################################################################################"
EOF
...
docker run --name "$CONTAINER_NAME" --user root --platform linux/x86_64 --entrypoint sh --mount type=bind,source=/Users/graham/Code/asystem/src/meg/postgres/src/main/resources/image,target=/asystem/etc,readonly -dt 'postgres:14.13-bullseye'
