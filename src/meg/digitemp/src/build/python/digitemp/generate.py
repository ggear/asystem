...
telegraf --once >/dev/null 2>&1 &&
        ...
        for metadata_digitemp_state_topic in metadata_digitemp_df["state_topic"].unique():
            metadata_digitemp_health_file.write("  " + """
  [ $(mosquitto_sub -h ${{VERNEMQ_SERVICE}} -p ${{VERNEMQ_API_PORT}} -t '{}' -W 1 2>/dev/null | jq -r .tags.run_code) -eq 0 ] &&
  [ $(mosquitto_sub -h ${{VERNEMQ_SERVICE}} -p ${{VERNEMQ_API_PORT}} -t '{}' -W 1 2>/dev/null | jq -r .fields.metrics_failed) -eq 0 ] &&
  [ $(mosquitto_sub -h ${{VERNEMQ_SERVICE}} -p ${{VERNEMQ_API_PORT}} -t '{}' -W 1 2>/dev/null | jq -r .fields.metrics_succeeded) -eq {} ]
            """.format(
