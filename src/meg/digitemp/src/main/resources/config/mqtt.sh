...
mosquitto_sub -h $VERNEMQ_IP -p $VERNEMQ_PORT --remove-retained -F '%t' -t 'homeassistant/+/digitemp/#' -W 1 2>/dev/null
...
mosquitto_sub -h $VERNEMQ_IP -p $VERNEMQ_PORT --remove-retained -F '%t' -t 'telegraf/+/digitemp/#' -W 1 2>/dev/null
