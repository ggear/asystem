...
if [ -n "${SHARE_DIR_MEDIA}" ]; then
  ...
  mkdir -p "${SHARE_DIR}/$(dirname "$(dirname "${SCRIPT_FILE}")")"
  chmod 777 "${SHARE_DIR}/$(dirname "$(dirname "${SCRIPT_FILE}")")"
  ...
  for _SHARE_DIR in ${SHARE_DIRS_LOCAL}; do
    mkdir -p "${_SHARE_DIR}/$(dirname "$(dirname "${SCRIPT_FILE}")")"
    chmod 777 "${_SHARE_DIR}/$(dirname "$(dirname "${SCRIPT_FILE}")")"
