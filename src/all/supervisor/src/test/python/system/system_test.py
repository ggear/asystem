...
TOPIC_CLUSTER_STATUS = "supervisor/all/status"
TOPIC_LEASE = "supervisor/all/leader/lease"
RIVAL = "macmini-mad"
TIMEOUT_ELECTION = 90


def _candidate(host):
    return "supervisor/all/leader/candidate/{}".format(host)
...
def test_wins_every_election_it_stands_in_alone():
    topics = [TOPIC_LEASE, _candidate(HOST), TOPIC_CLUSTER_STATUS]

    def assertion():
        received = _collect(topics, SETTLE)
        assert received.get(TOPIC_CLUSTER_STATUS) == b"online", "the cluster holder must mark the cluster online"
        assert _candidate(HOST) in received, "the host must stand in the cluster election"
        assert json.loads(received[_candidate(HOST)])["host"] == HOST, "a candidacy must name its own host"
        assert TOPIC_LEASE in received, "the sole candidate must claim the cluster lease"
        lease = json.loads(received[TOPIC_LEASE])
        assert lease["host"] == HOST, "the cluster lease must name the sole candidate, not [{}]".format(lease["host"])
...
def test_defers_to_an_incumbent_and_reclaims_when_it_leaves():
    ...
    try:
        ...
        clearer.loop_stop()
        clearer.disconnect()
        received, _, _ = _await([TOPIC_LEASE, TOPIC_CLUSTER], lambda _seen: False, 15, lambda _client: None)
        leases = [json.loads(payload)["host"] for payload in received.get(TOPIC_LEASE, []) if payload]
        assert leases, "the rival lease must be observed while it stands"
        assert _collect([TOPIC_CLUSTER_STATUS], SETTLE).get(TOPIC_CLUSTER_STATUS) == b"offline", \
            "a host that yields the cluster must mark the cluster offline, the rival publishes no presence"
    ...
    def assertion():
        received = _collect([TOPIC_LEASE, TOPIC_CLUSTER], SETTLE)
        assert TOPIC_LEASE in received, "the lease must be claimed again once the incumbent leaves"
        assert json.loads(received[TOPIC_LEASE])["host"] == HOST, "the remaining candidate must reclaim the lease"
        assert TOPIC_CLUSTER in received, "a new leader must republish the cluster status at once"
        assert _collect([TOPIC_CLUSTER_STATUS], SETTLE).get(TOPIC_CLUSTER_STATUS) == b"online", "the reclaiming holder must mark the cluster online"
...
def test_resumes_publishing_after_the_broker_restarts():
    ...
    held = _collect([TOPIC_LEASE], SETTLE).get(TOPIC_LEASE)
    assert held, "the cluster election must be held before the broker restarts"
    epoch_before = json.loads(held)["epoch"]
    assert _docker("restart", BROKER_CONTAINER).returncode == 0

    def assertion():
        received = _collect([TOPIC_STATUS, TOPIC_PROCESSOR, TOPIC_CLUSTER_STATUS, TOPIC_LEASE], SETTLE)
        assert received.get(TOPIC_CLUSTER_STATUS) == b"online", "the cluster must come back online after a broker restart"
        ...
        lease = json.loads(received[TOPIC_LEASE])
        assert lease["host"] == HOST, "the cluster election must be won again after the broker restarts"
        assert lease["epoch"] > epoch_before, "a lost session must settle and win a new epoch, not keep the old one"
...
def test_retains_its_records_across_a_graceful_stop():
    ...
    time.sleep(SETTLE)
    elections = [TOPIC_LEASE, _candidate(HOST)]
    ...
    assert _collect([TOPIC_CLUSTER_STATUS], SETTLE).get(TOPIC_CLUSTER_STATUS) == b"offline", "a resigning holder must mark the cluster offline"
...
def test_publishes_its_will_and_keeps_its_records_when_killed():
    ...
    def assertion():
        ...
        assert _collect([TOPIC_CLUSTER_STATUS], SETTLE).get(TOPIC_CLUSTER_STATUS) == b"offline", "the presence will must mark the cluster offline"
