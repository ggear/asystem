...
func TestProbeLibDrives_Wear(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			life, _, err := set.usedDriveLife()
			if testCase.expectedError {
				if err == nil {
					t.Fatalf("usedDriveLife: expected error but got nil")
				}
				return
			}
			if err != nil {
				t.Fatalf("usedDriveLife: unexpected error %v", err)
			}
			if life != testCase.expectedLife {
				t.Errorf("usedDriveLife: got %d want %d", life, testCase.expectedLife)
			}
