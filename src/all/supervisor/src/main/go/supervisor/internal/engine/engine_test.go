...
func TestEngine_CompareBarrier(t *testing.T) {
	tests := []struct {
		name     string
		barrier  *hostBarrier
		reaped   []string
		...
		{name: "happy no barrier for this reconcile", barrier: nil, reaped: nil, expected: "[none]"},
		{name: "sad barrier has not returned", barrier: &hostBarrier{}, reaped: []string{"svc-a"}, expected: "[pending]"},
		{name: "happy both reap nothing", barrier: &hostBarrier{returned: true}, reaped: nil, expected: "[agreed]"},
		{name: "happy both reap the same set", barrier: &hostBarrier{returned: true, shadow: []string{"svc-a", "svc-b"}}, reaped: []string{"svc-b", "svc-a"}, expected: "[agreed]"},
		{name: "sad timer reaps a service the barrier kept", barrier: &hostBarrier{returned: true}, reaped: []string{"svc-a"}, expected: "[differ]"},
		{name: "sad barrier reaps a service the timer kept", barrier: &hostBarrier{returned: true, shadow: []string{"svc-a"}}, reaped: nil, expected: "[differ]"},
	}
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			buffer := scribe.EnableBuffer(slog.LevelDebug, 64)
			t.Cleanup(scribe.Disable)
			barriers := map[string]*hostBarrier{}
			if testCase.barrier != nil {
				barriers["alpha"] = testCase.barrier
			}
			var guard sync.Mutex
			compareBarrier(barriers, &guard, "alpha", testCase.reaped, time.Now())
			found := false
			for _, line := range buffer.Tail(64) {
				if strings.Contains(line.Detail, testCase.expected) {
					found = true
				}
			}
			if !found {
				t.Errorf("compareBarrier: got no line containing %v", testCase.expected)
			}
		})
	}
}
