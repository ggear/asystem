...
func TestProbeImplHost_UsedProcessor(t *testing.T) {
	tests := []struct {
		name          string
...
func TestProbeImplHost_UsedMemory(t *testing.T) {
	tests := []struct {
		name           string
...
func TestProbeImplHost_Temperature(t *testing.T) {
	tests := []struct {
		name          string
...
func TestProbeImplHost_FailedLogs(t *testing.T) {
	t.Cleanup(resetLogs)
	probe := newHostProbe()
...
func TestProbeImplHost_FailedShares(t *testing.T) {
	tests := []struct {
		name          string
		expectedValue int8
		expectedOK    bool
...
func TestProbeImplHost_WarnTemperature(t *testing.T) {
	tests := []struct {
		name          string
		temps         map[string]float64
		expectedValue int8
...
func TestProbeImplHost_LifeUsedDrives(t *testing.T) {
	tests := []struct {
		name          string
		expectedValue int8
		expectedOK    bool
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			seedHostMounts(t)
			probe := newHostProbe()
			value, _, err := probe.usedDriveLife()
			if testCase.expectedError && err == nil {
				t.Fatalf("expected error but got nil")
			}
			if !testCase.expectedError && err != nil {
...
func TestProbeImplHost_UsedHomeSpace(t *testing.T) {
	tests := []struct {
		name          string
		expectedValue int8
		expectedOK    bool
...
func TestProbeImplHost_UsedShareSpace(t *testing.T) {
	tests := []struct {
		name          string
		expectedValue int8
		expectedOK    bool
...
func TestProbeImplHost_UsedSwapSpace(t *testing.T) {
	tests := []struct {
		name          string
		total         uint64
		used          uint64
...
func TestProbeImplHost_UsedDiskOps(t *testing.T) {
	tests := []struct {
		name          string
...
func TestProbeImplHost_UsedNetwork(t *testing.T) {
...
func TestProbeImplHost_RunningTime(t *testing.T) {
	tests := []struct {
		name          string
		seconds       uint64
		readerErr     error
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			probe := newHostProbe()
			probe.hostUptime = func() (uint64, error) { return testCase.seconds, testCase.readerErr }
			if testCase.noReader {
				probe.hostUptime = nil
			}
			value, _, err := probe.upTime()
			if testCase.expectedError != (err != nil) {
				t.Fatalf("error: got %v want %v", err, testCase.expectedError)
			}
			if err != nil {
...
func TestProbeImplHost_TemperatureDiscoversOnce(t *testing.T) {
	t.Cleanup(resetSensors)
	probe := newHostProbe()
