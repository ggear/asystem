...
func TestProbeImplServices_Services(t *testing.T) {
	tests := []struct {
		name                 string
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.validateFunc != nil {
				...
				for _, service := range services {
					...
					usedDiskTime, _, err := service.usedDiskTime()
					if err != nil {
						t.Fatalf("unexpected disk ops error: %v", err)
					}
					if usedDiskTime < 0 || usedDiskTime > 100 {
						t.Fatalf("usedDiskTime out of range [%d]", usedDiskTime)
					}
					...
					if err != nil {
						t.Fatalf("unexpected upTime error: %v", err)
					}
					if upTime < 0 {
...
func TestProbeImplServices_Health(t *testing.T) {
	tests := []struct {
		name          string
		healthScript  string
		expectHealthy bool
...
func TestProbeImplServices_Run(t *testing.T) {
	tests := []struct {
		name      string
	...
	}{
		{
			...
			checkFunc: func(t *testing.T, p *servicesProbe, cache *metric.RecordCache) {
				if err := p.poll(context.Background(), true); err != nil {
...
func TestProbeImplServices_Version(t *testing.T) {
	tests := []struct {
		name          string
...
func TestProbeImplServices_Sleep(t *testing.T) {
