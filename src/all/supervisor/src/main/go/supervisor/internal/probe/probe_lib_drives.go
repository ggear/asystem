...
func (s *mountSet) usedDriveLife() (int8, derivation, error) {
...
func (s *mountSet) worn(mounts []mountUsage, collectStart time.Time) ([]driveWear, time.Time) {
	...
	kernels := []string(nil)
	if previous != nil {
		kernels = make([]string, 0, len(previous.drives))
		for _, drive := range previous.drives {
			kernels = append(kernels, drive.kernel)
		}
	}
	if previous != nil && config.SinceIncludingSuspend(previous.read) < window && slices.Equal(physicals, kernels) {
		scribe.Log(scribe.SourceProbeDrives, scribe.SubjectMetric(metric.MetricHostUsedDriveLife), scribe.ActionSample).Debugf("retained", collectStart, "[%3d] drive readings aged [%s] within [%s], not re-read", len(previous.drives), config.SinceIncludingSuspend(previous.read).Truncate(time.Second), window)
...
func (s *mountSet) identifyFromReport(identity *driveIdentity, report smartReport, identifyStart time.Time) {
	...
	case !report.supported:
		...
		scribe.Log(scribe.SourceProbeDrives, scribe.SubjectMetric(metric.MetricHostUsedDriveLife), scribe.ActionSample).Infof("excluded", identifyStart, "[%s] at [%s] reports no smart support with [%s], not counted in wear", identity.kernel, identity.node, report.reason)
	case driveRatings[report.model] > 0:
		identity.rating = driveRatings[report.model]
		identity.rated = true
	default:
		identity.excluded = "model absent from the ratings"
		scribe.Log(scribe.SourceProbeDrives, scribe.SubjectMetric(metric.MetricHostUsedDriveLife), scribe.ActionSample).Infof("unlisted", identifyStart, "[%s] model [%s] absent from the ratings, not counted in wear", identity.kernel, report.model)
	}
}
