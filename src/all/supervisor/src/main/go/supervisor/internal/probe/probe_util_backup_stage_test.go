...
func TestProbeUtilBackupStage_VerdictRanksTheHaltCausesAboveTheRunResult(t *testing.T) {
	tests := []struct {
		...
		{name: "a_detached_disk_reads_as_stopped", cause: errStageDetached,
			expectedState: metric.BackupStateStopped, expectedSuccess: false},
		{name: "a_cancelled_parent_reads_as_stopped_over_the_error_the_cancel_caused", cause: context.Canceled, runErr: errors.New("share vanished"),
			expectedState: metric.BackupStateStopped, expectedSuccess: false},
		{name: "a_bare_cancelled_parent_reads_as_stopped", cause: context.Canceled,
			expectedState: metric.BackupStateStopped, expectedSuccess: false},
...
func TestProbeUtilBackupStage_RealStageStreamTakesTheExitStatusOfACommandLeavingAChildOnItsPipes(t *testing.T) {
	out, code, abandoned := realStageStream(context.Background(), nil, "sh", "-c", "echo started; sleep 3 & exit 0")
	if code != 0 || abandoned {
		t.Errorf("realStageStream: got code %d abandoned %v want 0 false", code, abandoned)
	}
