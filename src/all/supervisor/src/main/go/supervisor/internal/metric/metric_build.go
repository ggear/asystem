...
var metricBuildersByID = []builder{
	...
	MetricHostFailedBackupStages: {
		...
		label:        "Fail BKP",
		unit:         "%",
		description:  "backup stages failing outright against the stages this host runs",
		template:     "supervisor/$HOST/$SCOPE/host/failed_backup_stages",
		persisted:    true,
		dependencies: []ID{MetricHostHaltedBackupStages},
		pulseRule:    Bounded(Self, Exactly, 0),
		trendRule:    All(Bounded(Self, Exactly, 0), Healthy(MetricHostHaltedBackupStages)),
	},
	MetricHostHaltedBackupStages: {
		id:          MetricHostHaltedBackupStages,
		valueKind:   ValueInt,
		label:       "Halt BKP",
		unit:        "%",
		description: "backup stages halted by a stop or a timeout against the stages this host runs",
		template:    "supervisor/$HOST/$SCOPE/host/halted_backup_stages",
		persisted:   true,
		pulseRule:   Always(),
		trendRule:   Bounded(Self, Exactly, 0),
	},
