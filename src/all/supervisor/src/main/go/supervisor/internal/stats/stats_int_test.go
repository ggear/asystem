...
func TestIntStats_NewTrendWindow(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "happy_zero_duration_trend_off",
			durationHours: 0,
			tickFreqSec:   1,
			expectedPanic: false,
		},
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			if testCase.expectedPanic {
				...
				newTrendWindow(testCase.durationHours, testCase.tickFreqSec)
				return
			}
			window := newTrendWindow(testCase.durationHours, testCase.tickFreqSec)
			if window == nil {
...
func TestIntStats_NewPulseWindow(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			window := newPulseWindow(testCase.pulseSec, testCase.tickFreqSec)
			if window == nil {
				t.Fatal("Got window = nil, expected non-nil")
			}
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			if testCase.expectedPanic {
				...
				NewIntStats(testCase.trendHours, testCase.pulseSec, testCase.tickFreqSec)
				return
			}
			window := NewIntStats(testCase.trendHours, testCase.pulseSec, testCase.tickFreqSec)
			if window == nil {
				t.Fatal("Got window = nil, expected non-nil")
			}
			if (window.trend == nil) != testCase.expectedTrendNil {
				t.Fatalf("Got trend == nil = %v, expected %v", window.trend == nil, testCase.expectedTrendNil)
			}
		})
	}
}
...
func TestIntStats_TrendWindow_TickFrequencyVariations(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			window := newTrendWindow(24, testCase.tickFreqSec)
			for index := 0; index < testCase.ticks; index++ {
...
func TestIntStats_PulseWindow_Median(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			window := newPulseWindow(testCase.windowSize, 1)
			testCase.pushAndTick(window)
			median := window.median()
			if median != testCase.expectedMedian {
				t.Fatalf("Got median = %d, expected %d", median, testCase.expectedMedian)
...
func TestIntStats_TrendOff(t *testing.T) {
	tests := []struct {
		...
		expectedPulseLast   int8
		expectedPulseMean   int8
		expectedPulseMedian int8
		expectedPulseMax    int8
		expectedPulseMin    int8
	}{
		{
			name:      "happy_trend_off_returns_zero_before_push",
			setupFunc: func(w *IntStats) {},
		},
		{
			...
			expectedPulseLast:   100,
			expectedPulseMean:   75,
			expectedPulseMedian: 50,
			expectedPulseMax:    100,
			expectedPulseMin:    50,
		},
		{
			...
			expectedPulseLast:   50,
			expectedPulseMean:   30,
			expectedPulseMedian: 30,
			expectedPulseMax:    50,
			expectedPulseMin:    10,
		},
	}
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			window := NewIntStats(0, 5, 1)
			if window.trend != nil {
				t.Fatalf("Got trend != nil, expected nil for trendHours=0 (no memory should be allocated)")
			}
			testCase.setupFunc(window)
			if window.TrendMean() != 0 {
				t.Fatalf("Got TrendMean = %d, expected 0", window.TrendMean())
			}
			if window.TrendMedian() != 0 {
				t.Fatalf("Got TrendMedian = %d, expected 0", window.TrendMedian())
			}
			if window.TrendMax() != 0 {
				t.Fatalf("Got TrendMax = %d, expected 0", window.TrendMax())
			}
			if window.TrendMin() != 0 {
				t.Fatalf("Got TrendMin = %d, expected 0", window.TrendMin())
			}
			if window.TrendP95() != 0 {
				t.Fatalf("Got TrendP95 = %d, expected 0", window.TrendP95())
			}
			if window.PulseLast() != testCase.expectedPulseLast {
				t.Fatalf("Got PulseLast = %d, expected %d", window.PulseLast(), testCase.expectedPulseLast)
			}
			if window.PulseMean() != testCase.expectedPulseMean {
				t.Fatalf("Got PulseMean = %d, expected %d", window.PulseMean(), testCase.expectedPulseMean)
			}
			if window.PulseMedian() != testCase.expectedPulseMedian {
				t.Fatalf("Got PulseMedian = %d, expected %d", window.PulseMedian(), testCase.expectedPulseMedian)
			}
			if window.PulseMax() != testCase.expectedPulseMax {
				t.Fatalf("Got PulseMax = %d, expected %d", window.PulseMax(), testCase.expectedPulseMax)
			}
			if window.PulseMin() != testCase.expectedPulseMin {
				t.Fatalf("Got PulseMin = %d, expected %d", window.PulseMin(), testCase.expectedPulseMin)
			}
		})
	}
}
