...
type servicesProbe struct {
	...
	usedDiskTimeInt        map[string]*stats.IntStats
	usedNetworkInt         map[string]*stats.IntStats
	upTimeFloat            map[string]*stats.FloatStats
...
func newServicesProbe() *servicesProbe {
	...
	return &servicesProbe{
		...
		usedDiskTimeInt:      make(map[string]*stats.IntStats),
		usedNetworkInt:       make(map[string]*stats.IntStats),
		upTimeFloat:          make(map[string]*stats.FloatStats),
...
func (*servicesProbe) dormant() bool { return false }
...
func (p *servicesProbe) metrics() []metric.ID {
	return []metric.ID{
		...
		metric.MetricServiceUsedDiskRate,
...
func (p *servicesProbe) poll(ctx context.Context, isPulse bool) error {
	...
	usedDiskOpsInts := syncStatsFields(p.usedDiskTimeInt, polledServiceNames, newInt)
	usedNetworkInts := syncStatsFields(p.usedNetworkInt, polledServiceNames, newInt)
	upTimeFloats := syncStatsFields(p.upTimeFloat, polledServiceNames, newFloat)
...
func (p *servicesProbe) services(ctx context.Context, snapshot *installSnapshot) (map[string]service, error) {
	...
	for _, serviceContainer := range containers {
		...
		if err != nil {
			...
			usedDiskTime, diskOpsDerived, diskOpsErr := p.diskOpsUsed(name, fetchedRates)
			if diskOpsErr != nil {
				service.usedDiskOpsErr = diskOpsErr
			} else {
				service.usedDiskOpsValue = usedDiskTime
		...
		service.backupStatusValue, service.backupStatusDerived, service.backupStatusErr = p.backupStatus(name)
...
type service struct {
	...
	backupStatusDerived   derivation
...
func (s *service) backupStatus() (bool, derivation, error) {
	return s.backupStatusValue, s.backupStatusDerived, s.backupStatusErr
}
...
func (s *service) usedDiskTime() (int8, derivation, error) {
...
func (p *servicesProbe) ioRates(name, id string, response container.StatsResponse) serviceIORates {
	blockMoved, blockEntries := uint64(0), 0
	for _, entry := range response.BlkioStats.IoServiceBytesRecursive {
		operation := strings.ToLower(entry.Op)
		if operation != "read" && operation != "write" {
			continue
		}
		blockMoved += entry.Value
		blockEntries++
	}
	networkMoved := uint64(0)
	for _, network := range response.Networks {
		networkMoved += network.RxBytes + network.TxBytes
	}
	rates := serviceIORates{blockEntries: blockEntries, networkInterfaces: len(response.Networks)}
	taken := response.Read
	if taken.IsZero() {
...
func (p *servicesProbe) backupStatus(name string) (bool, derivation, error) {
