...
for DATABASE_NAME in $(databases); do
  ...
  while read -r SCHEMA_NAME; do
    ...
    while IFS=$'\t' read -r TABLE COLUMNS SIZE STAMP EPOCH; do
      [ -z "${TABLE}" ] && continue
      [ -n "$(printf '%s' "${HYPERTABLED}" | jq -r --arg s "${SCHEMA_NAME}" --arg t "${TABLE}" \
        'select(.schema == $s and .table == $t) | .table')" ] && continue
      extent "${STAMP}" "${EPOCH}"
    ...
    while IFS=$'\t' read -r TABLE SIZE; do
      [ -z "${TABLE}" ] && continue
      META="$(printf '%s' "${CATALOGUED}" | jq -r --arg s "${SCHEMA_NAME}" --arg t "${TABLE}" \
        'select(.schema == $s and .table == $t) | [.columns, .stamp, .epoch] | @tsv')"
